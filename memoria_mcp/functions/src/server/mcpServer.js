    ...
    try {
        switch (name) {
            ...
            case 'retrieve_context': {
                const context = await memoryLayer.retrieveContext(args?.query || '', args?.limit || 5, args?.filters);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'search_memories': {
                const results = await memoryLayer.searchMemories(args?.searchTerm || '', args?.searchType || 'hybrid', args?.limit || 10, userId);
                return {
                    content: [
                        {
                            type: 'text',
